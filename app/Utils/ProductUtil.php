...
class ProductUtil extends Util
{
    ...
    public function calculateInvoiceTotal($products, $tax_id, $discount = null, $uf_number = true)
    {
        ...
        if (!empty($tax_id)) {
          ...
          /* get product tax  add by ali 2022-1-14*/
                $porduct_tax=0;
                $output['product_tax']=0;
                foreach ($products as $product) {
                    $porduct_tax = $uf_number ? $this->num_uf($product['item_tax']) : $product['item_tax'];
                    $quantity = $uf_number ? $this->num_uf($product['quantity']) : $product['quantity'];
                    $output['product_tax'] += $quantity * $porduct_tax;
                }
              //$output['tax']=$output['tax']+$output['product_tax'];
             $output['total_before_tax']=$output['total_before_tax']-$output['product_tax'];
        /* End */
