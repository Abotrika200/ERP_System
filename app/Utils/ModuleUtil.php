...
class ModuleUtil extends Util
{
    ...
    public function log($business_id)
    {
                 $package = \Modules\Superadmin\Entities\Subscription::aclog($business_id);
    ...
    public static function logres($redirect_url = null)
    {
        $response_array = ['success' => 0,
                        'msg' => __(
                            "تم تخطي عدد المشاريع",
        ...
        if (request()->ajax()) {
            if (request()->wantsJson()) {
                ...
                return view('superadmin::subscription.log');
            }
        } else {
