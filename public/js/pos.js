$(document).ready(function() {
    ...
    * */
    ...
    function  calculat_tax(amount,percentage){
   ...
   /* ({
    ...
    $(document).on('click', '#cate_filter_btn', function() {
...
//Update values for each row    pos_unit_price_inc_tax
...
function pos_order_tax(price_total, discount) {
   ...
   /*
   order_tax =is tax on the (total- discount)
     tax on evry product ==item_tax
     add by ali 14-1-2022
           var product_tax=0;
            var quantity=0;
    $('table#pos_table tbody tr').each(function() {
        pos_quantity  =__read_number($(this).find('input.pos_quantity '));
        product_tax = product_tax + __read_number($(this).find('input.item_tax'))* pos_quantity;

    });
    order_tax=order_tax+product_tax;
  end 14-1-2022*/
